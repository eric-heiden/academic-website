...
- Start every new report from the [Prism research report template](static/reports/template-gallery/report.html).
- The complete reusable implementation, including styles, scripts, media examples, and a layout index, is in [`static/reports/template-gallery/`](static/reports/template-gallery/).
...
- Put each report in `static/reports/<report-name>/` unless an established report uses another documented location.
...
## Prism publication style

- Use the Prism report design as the default visual language.

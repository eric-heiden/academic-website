...
cuda.cuGraphGetNodes.argtypes = [C.c_void_p, C.POINTER(C.c_void_p), C.POINTER(C.c_size_t)]
...
n = C.c_size_t()
check(cuda.cuGraphGetNodes(e.mpc.graph.graph, None, C.byref(n)))
nodes = (C.c_void_p * n.value)()
check(cuda.cuGraphGetNodes(e.mpc.graph.graph, nodes, C.byref(n)))
types = Counter()
names = Counter()
copies = Counter()
for handle in nodes:
...
for mode in ("graph", "eager"):
    ...
    for repeat in range(5):
        ...
        if mode == "graph":
            ...
            e.mpc.optimize(e.solver.mjw_data.qpos, e.solver.mjw_data.qvel, e.solver.mjw_data.time)
...
print(json.dumps({k: v for k, v in results.items() if k != "kernel_names"}), flush=True)
